import { circleMarker, icon, layerGroup, LayerGroup, map, Map, marker, Marker, tileLayer } from 'leaflet';
import { filter, find, forEach, isEmpty, uniq } from 'lodash';
...
async function load_map(): Promise<void> {
    ...
    const mapbox_style_id = getDarkmode() ? mapbox_style_id_dark : mapbox_style_id_light;
    ...
    mymap = map('map').setView([aerodrome_data['latitude'], aerodrome_data['longitude']], 13);
...
async function update_map() {
    ...
    forEach(standstatus_data, (stand) => {
        ...
        let stand_text = "<div style='text-align: center;'>Stand <strong>" + stand['id'] + '</strong></div>';
