...
@push('custom-script')
    <script>
        let linkBaseUrl = "{{ route('administration.membership.groups') }}";

        const options = {
            currentPage: {{ $groups->currentPage() }},
            maxPage: {{ $groups->lastPage() }},
            ajaxSearchUrl: "{{ route('api.administration.membership.groups.search') }}",
            loadPaginatedUrl: "{{ route('api.administration.membership.groups.getpaginated') }}",

            list_content: $("#group-list-content"),
            search_input: $("#group-search-input"),
        };
        ...
        new Pagination(
            ...
            (value, container) => {
                ...
                container.append(`<tr class="text-center">
                        ...
                        <td>${value['name']}</td>
                        <td>
                            <a href="${linkBaseUrl + '/' + value['id']}"><button class="btn btn-sm btn-soft-primary p-1 px-3" style="font-size: 15px"><i class="mdi mdi-eye-outline"></i></button></a>
                        </td>
                    </tr>`);
            }
        );
