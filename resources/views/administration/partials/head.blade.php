...
<!-- Main Css -->
@auth
    <link href="@if (\Auth::user()->settings->dark_mode) {{ asset('css/app-admin-dark.css') }} @else {{ asset('css/app-admin.css') }} @endif" rel="stylesheet"
...
@else
    @if (isset($_COOKIE['color_scheme']) && $_COOKIE['color_scheme'] == 'dark')
        <link href="{{ asset('css/app-admin-dark.css') }}" rel="stylesheet" type="text/css" id="theme-opt" />
    @else
        <link href="{{ asset('css/app-admin.css') }}" rel="stylesheet" type="text/css" id="theme-opt" />
    @endif
...
{{-- Simplebar css --}}
<link rel="stylesheet" href="{{ asset('css/vendor/simplebar.css') }}" type="text/css" />
